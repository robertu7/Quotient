...
export function lineTotalMinor({
  quantity,
  unitPriceMinor,
}: LineAmountInput): number {
  if (!Number.isSafeInteger(quantity) || !Number.isSafeInteger(unitPriceMinor))
    throw new Error("Money and quantity must use integer storage units");
...
export function invoiceBalanceMinor(
  ...
  if (paid > totalMinor)
    throw new Error("Payments cannot exceed the invoice total");
...
export function assertAllocationAvailable(
  quoted: number,
  allocated: number,
  requested: number
): void {
  ...
  if (allocated + requested > quoted)
    throw new Error("Allocation exceeds the remaining quoted quantity");
}
