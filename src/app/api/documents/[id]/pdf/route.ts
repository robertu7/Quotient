...
export async function GET(
  ...
  page.drawText(safeText(document.number ?? "DRAFT"), {
    x: PAGE.width - PAGE.margin - 150,
    y: y - 25,
    size: 10,
    font: regular,
  ...
  if (document.lifecycle !== "draft")
    page.drawText(`Revision ${document.revision}`, {
      x: PAGE.width - PAGE.margin - 150,
      y: y - 40,
      size: 9,
      font: regular,
  ...
  y -= 80;
  page.drawText(safeText(document.senderSnapshot.name), {
    x: PAGE.margin,
    y,
    size: 14,
    font: bold,
  ...
  y -= 18;
  page.drawText(safeText(document.customerSnapshot.name), {
    x: PAGE.margin,
    y,
    size: 12,
    font: bold,
  ...
  y -= 22;
  page.drawText(safeText(document.title), {
    x: PAGE.margin,
    y,
    size: 16,
    font: bold,
  ...
  const dateText =
    document.type === "quotation"
      ? `Issued: ${document.issueDate ?? "Draft"}  Valid until: ${document.validUntil ?? "Not set"}`
      : `Issued: ${document.issueDate ?? "Draft"}  Due: ${document.dueDate ?? "Not set"}`;
  ...
  page.drawText("Amount", {
    x: PAGE.width - PAGE.margin - bold.widthOfTextAtSize("Amount", 8),
    y,
    size: 8,
    font: bold,
    color: rgb(1, 1, 1),
  ...
  for (const line of document.lines) {
    ...
    page.drawText(safeText(line.unit), {
      x: columns[2],
      y,
      size: 9,
      font: regular,
    });
    page.drawText(formatMoney(line.unitPriceMinor, document.project.currency), {
      x: columns[3],
      y,
      size: 9,
      font: regular,
    ...
    page.drawText(amountText, {
      x: PAGE.width - PAGE.margin - regular.widthOfTextAtSize(amountText, 9),
      y,
      size: 9,
      font: regular,
    });
  ...
  page.drawText(formatMoney(total, document.project.currency), {
    x: 505,
    y,
    size: 11,
    font: bold,
  ...
  if (document.type === "invoice") {
    ...
    page.drawText(
      `Paid ${formatMoney(paid, document.project.currency)}   Balance ${formatMoney(balance, document.project.currency)}`,
      { x: 340, y, size: 9, font: regular }
    );
    y -= 28;
  }
