...
test("creates, allocates, pays, and downloads project documents", async ({
  page,
}) => {
  ...
  await page
    ...
    .fill(`customer-${suffix}@example.com`);
  await page.getByRole("button", { name: "Create customer" }).click();
  await expect(
    page.getByRole("heading", { name: `Customer ${suffix}` })
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: `Project ${suffix}` })
  ).toBeVisible();
  ...
  await page
    ...
    .fill("Design and implementation");
  ...
  await expect(
    page.getByRole("definition").filter({ hasText: "$1,250.00" })
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("dialog", { name: "Issue this document?" })
  ).toBeVisible();
  ...
  await issue.click();
  await page
    .getByRole("dialog", { name: "Issue this document?" })
    .getByRole("button", { name: "Issue", exact: true })
    .click();
  await expect(
    page.getByRole("heading", { name: /^Q-\d{4}-\d{4}$/ })
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("definition").filter({ hasText: "$500.00" })
  ).toBeVisible();
  ...
  await expect(
    page.locator(".metric").filter({ hasText: "$300.00" })
  ).toBeVisible();
